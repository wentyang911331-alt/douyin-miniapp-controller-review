...
当前阶段正在推进 1.1.6 价格信息移除版本。

当前版本为 1.1.6，开发分支为 `codex/v1.1.6-remove-prices`。代码基线为 `v1.1.5`。`v1.1.5` 已提交、打标签并推送 GitHub 备份。

## 1.1.6 当前口径

1.1.6 只做价格信息移除，不做 UI 换肤。当前小程序运行界面不得展示价格列、价格排序、价格排行、价格显示列选项或金额 + `元`。

本次范围：

- 删除 `硬件规格对比` 中的价格字段和价格列。
- `显示列` 面板不再出现价格。
- 排行 / 排序入口不再出现价格。
- 硬件模块默认排序改为 `峰值母线`。
- 旧缓存中保存过 `price` 排序或隐藏列时，需要自动清理并回落到有效字段。
- 历史 HTML `controller-review(1).html` 保留，不作为当前运行入口和本次验收对象。

验收入口：`node scripts/verify-v1.1.6-remove-prices.js`。

1.1.5 已确认口径：抖音原生导航栏标题改为代码文字 `摩拓`；原生导航栏左侧小程序 logo 不能由页面代码替换，需要通过抖音开放平台品牌 / logo 配置处理。用户提供的 SVG 已处理为蓝色主题圆形 mark，右侧文字字标不进入生产页面。

1.1.5 已接入进入页 loading：UI 侧交付 HTML / CSS / JS 形式的动画代码，前端已转译为原生抖音小程序写法。loading 至少展示 1 秒；真实加载未完成时持续展示；loading 背后主页面使用高斯模糊或等效压暗。logo 笔画优先使用 Canvas V2 绘制，失败时用 view 组合动画兜底。当前静态数据页面以 `onReady` 作为真实 ready 点。
...
1. 1.1.6 当前正在移除价格信息，完成后需要在抖音开发者工具中预览硬件表格、显示列、排行 / 排序入口。
2. 如需替换原生导航栏左侧小程序 logo，需要在抖音开放平台 / 真实 appid 的品牌配置中上传 `assets/brand/motuo-mark-app-icon.png`。
3. 后续微调优先限制在新版本范围内，不改历史标签。
...
- 1.1.5 Loading 交付说明：`docs/V1_1_5_LOADING_HANDOFF.md`
- 1.1.6 价格移除验收脚本：`scripts/verify-v1.1.6-remove-prices.js`
- 1.1.5 自动验收脚本：`scripts/verify-v1.1.5-title-logo.js`
